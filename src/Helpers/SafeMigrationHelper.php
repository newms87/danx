...
class SafeMigrationHelper
{
    ...
    protected function setupTemporaryTable()
    {
        ...
        // PostgreSQL doesn't support LIKE syntax directly, create table with structure
        DB::statement("CREATE TABLE \"$this->tempTable\" (LIKE \"$this->table\" INCLUDING ALL)");
    }
    ...
    protected function swapInNewTable()
    {
        ...
        DB::transaction(function () {
            // PostgreSQL uses ALTER TABLE for renaming
            DB::statement("ALTER TABLE \"$this->table\" RENAME TO \"$this->backupTable\"");
            DB::statement("ALTER TABLE \"$this->tempTable\" RENAME TO \"$this->table\"");
        });
    ...
    public function lockTables($tables)
    {
        ...
        foreach($tables as $table => $lockType) {
            // PostgreSQL uses different lock modes
            $pgLockMode = $lockType === 'WRITE' ? 'ACCESS EXCLUSIVE' : 'ACCESS SHARE';
            $locks[] = "\"$table\" IN $pgLockMode MODE";
        }

        // PostgreSQL uses LOCK TABLE syntax
        DB::unprepared('LOCK TABLE ' . implode(', ', $locks));
    ...
    protected function setAutoIncrement($table, $id)
    {
        ...
        $sequence = "{$table}_{$this->primaryKey}_seq";
        DB::statement("SELECT setval('$sequence', $id, false)");
    ...
    {
        // PostgreSQL releases locks at the end of the transaction
        // No explicit UNLOCK statement is needed, but we'll commit the transaction
        DB::commit();
    ...
    public function optimize()
    {
        ...
        DB::statement("VACUUM ANALYZE \"$this->table\"");
